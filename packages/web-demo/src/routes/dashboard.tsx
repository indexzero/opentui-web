...
function computeLayout(cols: number, rows: number) {
  ...
  // 60/40 column split. Spark below gauges in the left col; log below plasma in the right col.
  ...
  const { hostRef, status, error, fps } = useOpentuiTerminal({
    ...
    draw: ({ buf, t, frame }) => {
      ...
      if (frame % 2 === 0) {
        ...
        if (history.current.length > histLen) history.current.shift()
      }
      drawBorder(buf, P.spark.x, P.spark.y, P.spark.w, P.spark.h, ACCENT_DIM, 'cpu history')
      const sparkY = P.spark.y + 2
      drawSparkline(buf, P.spark.x + 2, sparkY, histLen, history.current, ACCENT, PANEL_BG)
      if (P.spark.h >= 5) {
        drawSparkline(buf, P.spark.x + 2, sparkY + 2, histLen, history.current.map((v) => v * 0.7), CYAN, PANEL_BG)
      }
      if (P.spark.h >= 7) {
        drawSparkline(buf, P.spark.x + 2, sparkY + 4, histLen, history.current.map((v) => 1 - v), PURPLE, PANEL_BG)
      }
      if (P.spark.h >= 4) {
      ...
      const statusText = `wasm 283kb · ${cols}x${rows} · ${frame.toString().padStart(6, ' ')} frames`
